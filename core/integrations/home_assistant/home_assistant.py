...
class HomeAssistant:
    def __init__(self, integration_config: typing.Dict, ova: "OpenVoiceAssistant"):  # noqa: F821
        ...
        self.headers = {
            ...
            "Authorization": f"Bearer {acccess_token}",
        }
        ...
        try:
            ...
            logger.error(f"Failed to start Homeassistant integration: {e}")
    ...
    def get_history_period(
        ...
        resp = requests.get(
            f"{self.api}/history/period{ext}{params}", headers=self.headers
        )
        resp.raise_for_status()
        return resp.json()
    ...
    def get_states(self, entity_id: str = ""):
    ...
    def get_camera_proxy(self, camera_entity_id: str):
        resp = requests.get(
            f"{self.api}/camera_proxy/{camera_entity_id}", headers=self.headers
        )
        resp.raise_for_status()
        return resp.content

    def get_calendars(self, calendar_entity_id: str = ""):
    ...
    def post_states(self, entity_id: str, state: typing.Dict):
        return requests.post(
            f"{self.api}/states/{entity_id}", headers=self.headers, json=state
        )

    def post_events(self, event_type: str, event_data: typing.Dict = None):
        if event_data:
            return requests.post(
                f"{self.api}/events/{event_type}", headers=self.headers, json=event_data
            )
        else:
            return requests.post(
                f"{self.api}/events/{event_type}", headers=self.headers
            )

    def post_services(
        self, domain: str, service: str, service_data: typing.Dict = None
    ):
        if service_data:
            return requests.post(
                f"{self.api}/services/{domain}/{service}",
                headers=self.headers,
                json=service_data,
            )
        else:
            return requests.post(
                f"{self.api}/services/{domain}/{service}", headers=self.headers
            )

    def post_template(self, template: typing.Dict):
        return requests.post(
            f"{self.api}/template", headers=self.headers, json=template
        )

    def post_config_core_check_config(self):
        return requests.post(
            f"{self.api}/config/core/check_config", headers=self.headers
        )

    def post_intent_handle(self, data: typing.Dict):
        return requests.post(
            f"{self.api}/intent/handle", headers=self.headers, json=data
        )
