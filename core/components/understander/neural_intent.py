...
class IntentClassifier(nn.Module):
    def __init__(self, dropout, hidden_dim, n_layers, vocab_size, num_classes):
        ...
        self.lstm = nn.LSTM(
            100, hidden_dim, n_layers, batch_first=True, bidirectional=True
        )
    ...
    def forward(self, x):
        ...
        out = torch.cat(
            (out[:, -1, : self.hidden_dim], out[:, 0, self.hidden_dim :]), dim=1
        )
        out = self.drop(out)
        out = self.fc(out)
        return out
...
class NeuralIntent:
    def __init__(
        ...
        if batch_size > 64:
            batch_size = 64
            config.set(
                Components.Understander.value, "config", "batch_size", batch_size
            )
        elif batch_size < 8:
            batch_size = 8
            config.set(
        ...
        if os.path.exists(vocab_file):
            (
                ...
                loaded_labels,
                self.max_length,
            ) = pickle.load(open(vocab_file, "rb"))

        if (
            retrain
            or not os.path.exists(vocab_file)
            or not os.path.exists(model_file)
            or sorted(labels) != sorted(loaded_labels)
        ):
            ...
            X, Y = preprocess_data(
                x, y, self.word_to_int, self.max_length, label_to_int
            )

            self.intent_model = IntentClassifier(
                dropout, hidden_dim, n_layers, n_vocab, n_labels
            ).to(self.device)
            train_classifier(
                X,
                Y,
...
def train_classifier(
    X, Y, minimum_training_accuracy, batch_size, model, model_file, device
):
    try:
        ...
        try:
            ...
            while accuracy < minimum_training_accuracy:
                ...
                while epoch < num_epochs:
                    ...
                    logger.info(
                        f"Epoch {epoch}/{num_epochs} | Train Loss: {avg_loss:.4f} | Accuracy: {accuracy:.2f}%"
                    )
...
def default_config() -> typing.Dict:
    return {
        ...
        "network_size": "small",
        "network_size_options": ["small", "medium", "large"],
        "dropout": 0.5,
        "batch_size": 8,
    }
