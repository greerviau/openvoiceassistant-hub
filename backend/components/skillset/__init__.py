...
class Skillset:
    ...
    def add_skill_config(self, skill: str, config: dict): # TODO use TypedDict
        if not self.skill_imported(skill):
            self.__import_skill(skill, config)
        else:
            raise RuntimeError('Skill is already imported')
    ...
    def get_skill_config(self, skill: str) -> dict: # TODO use TypedDict
        if self.skill_imported(skill):
    ...
    def run_stage(self, context: dict):  # TODO use TypedDict
        skill = context['skill']
    ...
    def __import_skill(self, skill: str, config: dict):  # TODO use TypedDict
